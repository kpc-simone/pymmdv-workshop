...
def plot_freezing_bouts(ax,trajectory_dir,color,speed_thresh=10,min_bout_duration=1.0):
    ...
    for t,trajectory_filename in enumerate(trajectory_filenames):
        trdf = pd.read_csv(os.path.join(trajectory_dir,trajectory_filename))
        ...
        frdf = stdf[stdf['bout duration'] > min_bout_duration]
        print('total time freezing for {} : {}'.format(animal_labels[-1],frdf['bout duration'].sum()))
    
        plot_bouts(ax,frdf,color,start_h = t)
...
if __name__ == '__main__':
    ...
    speed_thresh = 5            # assumes mm/s
    min_bout_duration = 1.0     # seconds

    plt.style.use('../multipanel_style.mplstyle')
    fig,ax = plt.subplots(1,1)
    plot_freezing_bouts(ax,trajectory_dir,color,speed_thresh=speed_thresh,min_bout_duration=min_bout_duration)
    fig.tight_layout()
    plt.show()
